  ...
  return (
    <ScrollView style={s.screen} contentContainerStyle={s.container} showsVerticalScrollIndicator={false}>
      ...
      <View style={{ height: 48 }} />
                ...
                <Text style={s.rowLabel}>My Tasks</Text>
                <Text style={s.rowDesc}>AI uses these to build your personalized weekly schedule</Text>
                    ...
                    <>
                        ...
                        {tasks.map(task => (
                            <View key={task._id} style={s.taskRow}>
                                ...
                                <View style={[s.priorityBadge, { backgroundColor: PRIORITY_COLORS[task.priority] + '22' }]}>
                        ...
                        {showAddForm && (
                            <View style={s.addForm}>
                                ...
                                <View style={s.priorityRow}>
                                    {(['high', 'medium', 'low'] as Priority[]).map(p => (
                                        <TouchableOpacity
                                            ...
                                            activeOpacity={0.75}
                                        >
                                            <Text style={[s.chipTxt, newPriority === p && { color: colors.white }]}>
                                ...
                                <View style={s.formBtns}>
                                    ...
                                    >
                                        <Text style={s.chipTxt}>Cancel</Text>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                        style={[s.chipOn, { flex: 1, alignItems: 'center', paddingVertical: spacing.sm, borderRadius: radii.full, opacity: (!newTaskName.trim() || addingTask) ? 0.5 : 1 }]}
                                        onPress={addTask}
                                        disabled={!newTaskName.trim() || addingTask}
                                    >
                                        {addingTask
                                            ? <ActivityIndicator color={colors.white} size="small" />
                                            : <Text style={s.chipTxtOn}>Add Task</Text>}
                                    </TouchableOpacity>
                                </View>
                            </View>
                        )}
